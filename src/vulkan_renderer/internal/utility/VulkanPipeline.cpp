...
namespace Common {
	...
	bool VulkanPipeline::LoadShader(SHADER_TYPE type, std::string name, bool spirv)
	{
		...
		path = "Resources/Shaders/Vulkan/Compiled/" + name;
	...
	int VulkanPipeline::Compile()
	{
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Creating pipeline layout failed";
			std::cout << s << std::endl;
			return res;
		}
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Creating pipeline failed";
			std::cout << s << std::endl;
			return res;
		}
	...
	uint32_t VulkanPipeline::GetPipelineId()
	{
		return pipelineId;
	}
	...
	int VulkanPipeline::CompileShaderModule(std::vector<char> code, VkShaderModule * shaderModule)
	{
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Creating shader module failed";
			std::cout << s << std::endl;
		}
