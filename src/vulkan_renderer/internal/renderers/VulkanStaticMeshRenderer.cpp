#include "vulkan_renderer/internal/renderers/VulkanStaticMeshRenderer.hpp"

#ifdef USING_VULKAN

#include "vulkan_renderer/internal/utility/Renderer.hpp"
...
namespace Common {
	...
	VulkanStaticMeshRenderer::VulkanStaticMeshRenderer(VulkanRenderer * renderer, VulkanLogicalDevice * device, VulkanDescriptorPool * descriptorPool)
	{
		...
		shadowPipeline_->SetRasterizerSettings(VK_TRUE, VK_FALSE,
			VK_POLYGON_MODE_FILL, 1.f, VK_CULL_MODE_NONE, VK_FRONT_FACE_COUNTER_CLOCKWISE, VK_TRUE, 0.01f, 0.f, 0.f);
...
/*
	...
	void VulkanStaticMeshRenderer::Clean()
	{
