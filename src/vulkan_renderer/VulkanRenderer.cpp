...
namespace Common
{
	...
	static void check_vk_result(VkResult err) {
		if (err != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(err) + "] ImGui vulkan call failed";
			std::cout << s << std::endl;
		}
	}
	...
	{
		InitializeRenderer(Common::Engine::GetEngine().lock()->GetWindow<VulkanWindow>());
	}
...
/*
	...
	void VulkanRenderer::RecreateSwapChain()
	{
		...
		vulkanInstancedSkeletalMeshRenderer->Clean();
		...
		vulkanInstancedSkeletalMeshRenderer->Recreate();
	...
	VkRenderPass VulkanRenderer::GetGBufferRenderPass()
	{
		return gBufferPass;
	}
	...
	VkCommandPool VulkanRenderer::GetGraphicsCommandPool()
	{
		return graphicsCommandPool;
	}

	VkCommandPool VulkanRenderer::GetComputeCommandPool()
	{
	...
	void VulkanRenderer::RendererBegin(const glm::mat4x4 & view, const glm::mat4x4 & projection)
	{
		...
		if (res == VK_ERROR_OUT_OF_DATE_KHR) {
			...
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Failed to aquire next swap chain image";
			std::cout << s << std::endl;
			return;
		}
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Queue wait idle failed";
			std::cout << s << std::endl;
		}*/
		...
		vulkanInstancedSkeletalMeshRenderer->StartRender(view, projection);
	...
	void VulkanRenderer::Render(const glm::mat4x4 & modelMatrix, std::shared_ptr<Model> model, const glm::vec4 & mainColor)
	{
		...
		for (size_t i = 0, size = meshes.size(); i < size; i++) {
			...
			if (!std::dynamic_pointer_cast<VulkanMesh, Mesh>(meshes[i])->isAnimated() || 
				...
				if (!renderInstancedSkeletons) {
					...
					vulkanInstancedSkeletalMeshRenderer->RenderMesh(modelMatrix,
						static_cast<VulkanMesh*>(meshes[i].get()),
						material.get(), model->GetSkeleton().get(), model->GetCurrentAnimationIndex(), model->GetAnimationTime(),
						model->GetSkeleton()->GetAnimationTicksPerSecond(model->GetCurrentAnimationIndex()),
						model->GetAnimationDuration(), model->IsLooping(), mainColor);
	...
	void VulkanRenderer::RendererEnd()
	{
		...
		if (res != VK_SUCCESS) {
			std::string s = "[CODE:" + std::to_string(res) + "] Failed to submit compute commands";
			std::cout << s << std::endl;
		}
		...
		vulkanInstancedSkeletalMeshRenderer->FinishRender(0, graphicsCommandPool, (instancedSkeletalMeshCommandBuffers_[currentImage]));
		//}
		...
		vkCmdExecuteCommands(currentBuffer_, 1, &instancedSkeletalMeshCommandBuffers_[currentImage]);
		...
		size = static_cast<uint32_t>(instancedSkeletalMeshShadowCommandBuffers_[currentImage].size());

		if (size < static_cast<uint32_t>(activeLights)) {
			instancedSkeletalMeshShadowCommandBuffers_[currentImage].resize(activeLights);
			GenerateSecondaryCommandBuffers(instancedSkeletalMeshShadowCommandBuffers_[currentImage].data() + sizeof(VkCommandBuffer)*size,
				static_cast<uint32_t>(instancedSkeletalMeshShadowCommandBuffers_[currentImage].size()) - size);
		}
		...
		for (int i = 0; i < activeLights; ++i) {
			if (!(lightData[i].position.w == 0.f && lightData[i].direction == glm::vec4(0.f))) {
				...
				vulkanInstancedSkeletalMeshRenderer->RenderShadows(0, graphicsCommandPool, (instancedSkeletalMeshShadowCommandBuffers_[currentImage][i]), static_cast<uint32_t>(i * sizeof(Light)));
				//}
			}
		...
		for (int i = 0; i < activeLights; ++i) {
			...
			if (!(lightData[i].position.w == 0.f && lightData[i].direction == glm::vec4(0.f))) {
				...
				vkCmdExecuteCommands(currentBuffer_, 1, &(instancedSkeletalMeshShadowCommandBuffers_[currentImage][i]));
			}
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Failed to record primary command buffer";
			std::cout << s << std::endl;
			return;
		}
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Submitting command buffer to queue failed";
			std::cout << s << std::endl;
			return;
		}
		...
		if (res == VK_ERROR_OUT_OF_DATE_KHR || res == VK_SUBOPTIMAL_KHR) {
			...
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Failed to present swap chain image";
			std::cout << s << std::endl;
		}
	...
	void VulkanRenderer::CreateSwapChain()
	{
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Failed to create new swapchain";
			std::cout << s << std::endl;
		}
	...
	void VulkanRenderer::CreateSwapChainImageViews()
	{
		...
		for (int i = 0; i < static_cast<int>(swapChainImages.size()); ++i) {
			...
			if (res != VK_SUCCESS) {
				std::string s = "[CODE:" + std::to_string(res) + "] Image view creation creation failed";
				std::cout << s << std::endl;
			}
		}
	}
	...
	void VulkanRenderer::CreateVmaAllocator()
	{
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Failed to create vulkan memory allocator";
			std::cout << s << std::endl;
		}
	}
	...
	void VulkanRenderer::CreateDepthImage()
	{
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Failed to create depth image";
			std::cout << s << std::endl;
		}
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Failed to create depth image view";
			std::cout << s << std::endl;
		}
	...
	void VulkanRenderer::CreateAttachmentImages()
	{
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Failed to create imgui image";
			std::cout << s << std::endl;
		}
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Failed to create imgui image view";
			std::cout << s << std::endl;
		}
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Failed to create render image";
			std::cout << s << std::endl;
		}
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Failed to create render image view";
			std::cout << s << std::endl;
		}
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Failed to create albedo image";
			std::cout << s << std::endl;
		}
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Failed to create albedo image view";
			std::cout << s << std::endl;
		}
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Failed to create position image";
			std::cout << s << std::endl;
		}
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Failed to create position image view";
			std::cout << s << std::endl;
		}
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Failed to create normal image";
			std::cout << s << std::endl;
		}
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Failed to create normal image view";
			std::cout << s << std::endl;
		}
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Failed to create ssao image";
			std::cout << s << std::endl;
		}
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Failed to create ssao image view";
			std::cout << s << std::endl;
		}
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Failed to create ssaoBlur image";
			std::cout << s << std::endl;
		}
		...
		if (res != VK_SUCCESS) {
			std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Failed to create ssaoBlur image view";
			std::cout << s << std::endl;
		}
	...
	void VulkanRenderer::CreateFramebuffers()
	{
		...
		for (int i = 0; i < framebuffers.size(); ++i) {
			...
			if (res != VK_SUCCESS) {
				std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Framebuffer creation failed";
				std::cout << s << std::endl;
			}
		}
	...
	void VulkanRenderer::GenerateCommandBuffers()
	{
		...
		instancedSkeletalMeshCommandBuffers_.resize(primaryRenderCommandBuffers_.size());
		...
		instancedSkeletalMeshShadowCommandBuffers_.resize(primaryRenderCommandBuffers_.size());
		...
		GenerateSecondaryCommandBuffers(instancedSkeletalMeshCommandBuffers_.data(), static_cast<uint32_t>(instancedSkeletalMeshCommandBuffers_.size()));
	...
	void VulkanRenderer::CreateDescriptorPool()
	{
		...
		vulkanDescriptorPools_[0]->addPoolSize(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 2000);
		...
		vulkanDescriptorPools_[0]->compile(12 * 1000);
	}
	...
	void VulkanRenderer::InitImGui()
	{
		...
		ImGui_ImplGlfwVulkan_Init(window.lock()->GetGLFWWindow().lock().get(), false, &initData);
	...
	void VulkanRenderer::CreateSemaphores()
	{
		...
		for (size_t i = 0, size = framebuffers.size(); i < size; ++i) {
			...
			if (res != VK_SUCCESS) {
				std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Semaphore 'image available' creation failed";
			...
			if (res != VK_SUCCESS) {
				std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Semaphore 'render finished' creation failed";
				debug_error("VulkanRenderer", "CreateSemaphores", s);
			}
			res = vkCreateSemaphore(vulkanLogicalDevice_->GetDevice(), &semaphoreInfo, nullptr, &computeFinishedSemaphores_[i]);
			if (res != VK_SUCCESS) {
				std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Semaphore 'compute finished' creation failed";
				debug_error("VulkanRenderer", "CreateSemaphores", s);
			}
			...
			if (res != VK_SUCCESS) {
				std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Fence 'drawing finished' creation failed";
				debug_error("VulkanRenderer", "CreateSemaphores", s);
			}

			res = vkCreateFence(vulkanLogicalDevice_->GetDevice(), &fenceInfo, nullptr, &computeFinishedFences_[i]);
			if (res != VK_SUCCESS) {
				std::string s = "[ERROR] [CODE:" + std::to_string(res) + "] Fence 'compute finished' creation failed";
				debug_error("VulkanRenderer", "CreateSemaphores", s);
			}
	...
	void VulkanRenderer::CreateRenderers()
	{
		...
		vulkanInstancedSkeletalMeshRenderer = std::unique_ptr<VulkanInstancedSkeletalMeshRenderer>(new VulkanInstancedSkeletalMeshRenderer(this, vulkanLogicalDevice_.get(), vulkanDescriptorPools_[0].get()));

		renderInstancedSkeletons = true;
	}
	...
	void VulkanRenderer::AllocateThreads()
	{
		...
		for (size_t i = 0, size = threads.size(); i < size; ++i) {
			...
			vulkanDescriptorPools_[i + 1]->addPoolSize(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 2000);
			...
			vulkanDescriptorPools_[i + 1]->compile(12 * 1000);
	...
	void VulkanRenderer::DestroyRenderers()
	{
		...
		vulkanInstancedSkeletalMeshRenderer.reset();
	}
	...
	void VulkanRenderer::StartSecondaryCommandBufferRecording(VkCommandBuffer buffer, VkCommandBufferUsageFlags flags, VkRenderPass renderPass, uint32_t subPass, VkFramebuffer framebuffer)
	{
	...
	void VulkanRenderer::EndSecondaryCommandBufferRecording(VkCommandBuffer buffer)
	{
